...
ENDPOINT=$(azd env get-value VLLM_PROXY_ENDPOINT 2>/dev/null)
MODEL=$(azd env get-value SERVED_MODEL_NAME 2>/dev/null)
...
echo ""
echo "  vLLM serves via continuous batching, so concurrent requests are handled natively."
echo ""
