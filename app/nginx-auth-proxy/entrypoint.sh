...
# -i reads the password from stdin so it never appears in argv (ps/proc) or shell history.
printf '%s' "$BASIC_AUTH_PASSWORD" | htpasswd -ci /etc/nginx/.htpasswd "$BASIC_AUTH_USER"
